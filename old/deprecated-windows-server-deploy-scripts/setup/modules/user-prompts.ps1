...
function Get-SiteName {
    ...
    Write-Host "A 'site' is a deployed instance with its own domain, data, and config." -ForegroundColor Gray
    Write-Host "(e.g., api-demo.com, api-staging.example.com)" -ForegroundColor Gray
    Write-Host ""
    Write-Host "Each site chooses which backend 'app' to run (see app/apps/):" -ForegroundColor Gray
    Write-Host "  - postgres_template (PostgreSQL-based API)" -ForegroundColor Gray
    Write-Host "  - mongodb_template (MongoDB-based API)" -ForegroundColor Gray
    Write-Host "  - demo_app, template_app, mongo_db_test" -ForegroundColor Gray
    Write-Host ""
    ...
    if (Test-Path $deploymentsDir) {
        ...
        if ($sites.Count -gt 0) {
            Write-Host ""
            Write-Host "Existing site deployments:" -ForegroundColor Yellow
            foreach ($site in $sites) {
                Write-Host "  - $site" -ForegroundColor Gray
            }
        } else {
            Write-Host ""
            Write-Host "No existing sites found. Create a new one or use available apps:" -ForegroundColor Yellow
            Write-Host "  - postgres_template" -ForegroundColor Gray
            Write-Host "  - mongodb_template" -ForegroundColor Gray
        }
    }
