...
_profile_keycloak_summary() {
    ...
    echo "service roots, and the profile test-user lifecycle using deployment"
    echo "values as defaults. Changes are validated, saved to root .env, and"
    ...
    echo "Application roles and secret-free test identities come only from the"
    echo "site profile. Missing test-user passwords are requested without echo"
